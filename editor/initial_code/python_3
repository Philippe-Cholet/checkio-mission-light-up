...
def light_up(grid: Tuple[str]) -> Iterable[Tuple[int]]: # or Iterable[List[int]]
    pass
...
if __name__ == '__main__':
    def checker(function, grid):
        ...
        digits = ((i, j, int(cell)) for i, row in enumerate(grid)
        ...
        for i, j in user_lights:
            if grid[i][j] == LIT: # LIGHTS CONFLICT!
                ...
                return False
            if grid[i][j] == WALL or grid[i][j].isdigit():
